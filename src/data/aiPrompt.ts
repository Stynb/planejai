...
export function buildAIChatPrompt(params: {
  simulation: SimulationRecord;
  userQuestion: string;
  insightJson?: string;
  previousMessages?: { role: "user" | "assistant"; content: string }[];
}) {
  const { simulation, userQuestion, insightJson, previousMessages } = params;

  const context = buildAIPrompt(simulation);
  const history = (previousMessages ?? [])
    .slice(-8)
    .map((m) => `${m.role === "user" ? "Usuário" : "Assistente"}: ${m.content}`)
    .join("\n");

  return `Você é um educador financeiro. Responda às perguntas do usuário de forma clara, prática e objetiva, em português do Brasil, sem markdown.

Contexto da simulação (use como base):
${context}

${insightJson ? `Insight já gerado (JSON):\n${insightJson}\n` : ""}
...
Regras:
- Responda em 3 a 8 linhas, com passos práticos quando fizer sentido
- Não retorne JSON; retorne texto puro
- Se faltar dado, explique o que está faltando e sugira como estimar
`;
}
